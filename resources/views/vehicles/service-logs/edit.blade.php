<x-app-layout title="Edit Service Log">
    <div class="space-y-6">
        ...
        <div class="bg-[var(--color-bg-primary)] rounded-xl shadow-lg border border-[var(--color-border-primary)] p-8">
            ...
            <form method="POST" action="{{ route('families.vehicles.service-logs.update', ['family' => $family->id, 'vehicle' => $vehicle->id, 'serviceLog' => $serviceLog->id]) }}">
                @csrf
                @method('PATCH')
                @include('vehicles.service-logs._form')
                <div class="mt-6 flex gap-3">
                    <x-button type="submit" variant="primary" size="md">Update Service Log</x-button>
                    <a href="{{ route('families.vehicles.service-logs.index', ['family' => $family->id, 'vehicle' => $vehicle->id]) }}" class="px-4 py-2 rounded-lg border border-[var(--color-border-primary)] text-[var(--color-text-primary)] hover:bg-[var(--color-bg-secondary)] transition-colors">Cancel</a>
                </div>
            </form>
        </div>
    </div>
