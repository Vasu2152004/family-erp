            ...
            @if($tasks->count() > 0)
                <div class="mt-6 grid grid-cols-1 gap-4">
                    @foreach($tasks as $task)
                        <div class="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border-primary)] p-5 hover:shadow-lift transition-all">
                            <div class="flex items-start justify-between">
                                <a href="{{ route('families.tasks.show', ['family' => $family->id, 'task' => $task->id]) }}" class="flex-1">
                                ...
                                <div class="flex items-center gap-2 ml-4">
                                    ...
                                    @if(in_array($task->id, $canDeleteIds ?? []))
                                        <x-form 
                                            method="DELETE" 
                                        ...
                                        >
                                            <x-button type="submit" variant="danger-outline" size="sm" onclick="event.stopPropagation();" title="Delete Task">
    ...
    @push('scripts')
        ...
        <script>
            (function() {
                ...
                function init() {
                    if (run()) { setTimeout(function() { initTaskCharts(taskStatusData); }, 500); return; }
                    var attempts = 0;
                    var t = setInterval(function() {
                        if (run()) { clearInterval(t); setTimeout(function() { initTaskCharts(taskStatusData); }, 500); return; }
