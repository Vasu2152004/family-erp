<x-app-layout title="Edit Event: {{ $family->name }}">
    <div class="space-y-6">
        <x-breadcrumb :items="[
        ...
        <div class="bg-[var(--color-bg-primary)] rounded-xl shadow-lg border border-[var(--color-border-primary)] p-6">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-bold text-[var(--color-text-primary)]">Edit Event</h1>
            </div>

            <form method="POST" action="{{ route('families.calendar.update', ['family' => $family->id, 'event' => $event->id]) }}" class="space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <x-input type="text" id="title" name="title" value="{{ old('title', $event->title) }}" class="mt-1 w-full" required />
                    @error('title') <p class="mt-1 text-sm text-[var(--color-error)]">{{ $message }}</p> @enderror
                </div>

                <div>
                    <x-label for="description">Description</x-label>
                    <textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-lg border border-[var(--color-border-primary)] px-4 py-2.5 text-[var(--color-text-primary)] bg-[var(--color-bg-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]">{{ old('description', $event->description) }}</textarea>
                ...
                <div>
                    ...
                    <select id="reminder_before_minutes" name="reminder_before_minutes" class="mt-1 block w-full rounded-lg border border-[var(--color-border-primary)] px-4 py-2.5 text-[var(--color-text-primary)] bg-[var(--color-bg-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]">
                        ...
                        @foreach([5, 10, 15, 30, 60, 120, 1440] as $minutes)
                            <option value="{{ $minutes }}" {{ (string) old('reminder_before_minutes', $event->reminder_before_minutes) === (string) $minutes ? 'selected' : '' }}>
                ...
                <div class="flex items-center gap-3">
                    <x-button type="submit" variant="primary" size="md">Update Event</x-button>
