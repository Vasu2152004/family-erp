<x-app-layout title="Edit Budget: {{ $family->name }}">
    <div class="space-y-6">
        <x-breadcrumb :items="[
        ...
        <div class="bg-[var(--color-bg-primary)] rounded-xl shadow-lg border border-[var(--color-border-primary)] p-8">
            <div class="mb-6">
                <h1 class="text-3xl font-bold text-[var(--color-text-primary)]">Edit Budget</h1>
                <p class="mt-2 text-sm text-[var(--color-text-secondary)]">
                    Update budget for {{ $family->name }}
                </p>
            </div>
        ...
        <form method="POST" action="{{ route('finance.budgets.update', ['budget' => $budget->id, 'family_id' => $family->id]) }}" class="space-y-6">
            ...
            <div>
                ...
                <select name="family_member_id" id="family_member_id" class="mt-1 block w-full rounded-lg border border-[var(--color-border-primary)] px-4 py-2.5 text-[var(--color-text-primary)] bg-[var(--color-bg-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]">
                    ...
                    @foreach($members as $member)
                        <option value="{{ $member->id }}" {{ old('family_member_id', $budget->family_member_id) == $member->id ? 'selected' : '' }}>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $budget->is_active) ? 'checked' : '' }} class="rounded border-[var(--color-border-primary)] text-[var(--color-primary)] focus:ring-[var(--color-primary)]">
